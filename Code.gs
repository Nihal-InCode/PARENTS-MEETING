 ...
 * - ?action=classes           â†’ JSON array of class names
 * - ?action=students&class=X  â†’ JSON array of student display strings
 * - no action                 â†’ serves the HTML UI (legacy Apps Script host)
 */
function doGet(e) {
...
/**
 * Web app entry point (POST) â€” attendance submit from Vercel /api/submit.
